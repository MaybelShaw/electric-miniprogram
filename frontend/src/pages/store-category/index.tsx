...
export default function StoreCategoryPage() {
  ...
  const loadCategoryDetail = async () => {
    ...
    try {
      ...
      setProductTotal((detail.products || []).length)
  ...
  const loadBrandProducts = async (brandName: string, pageNum = 1) => {
    ...
    try {
      ...
      setProductTotal(res.total ?? res.results.length)
  ...
  const handleAllBrandClick = async () => {
    if (loading || !selectedBrandName) return
    await loadCategoryDetail()
  }
      ...
      <ScrollView className='store-category-scroll' scrollY onScrollToLower={loadMoreProducts}>
        <View className='category-hero'>
          <View className='category-hero-main'>
            ...
            <View className='category-summary'>
              <Text>{selectedBrandName ? `品牌：${selectedBrandName}` : '全部品牌'}</Text>
              <Text>{productTotal} 件商品</Text>
            </View>
          </View>
        </View>

        <View className='brand-filter-section'>
          {brands.length > 0 ? (
            <ScrollView className='brand-scroll' scrollX scrollWithAnimation>
              <View className='brand-track'>
                <View
                  className={`brand-chip ${!selectedBrandName ? 'active' : ''}`}
                  onClick={handleAllBrandClick}
                >
                  <Text className='brand-chip-name'>全部</Text>
                </View>
                {brands.map((brand) => {
                  ...
                  return (
                    ...
                    >
                      {resolveLocalMediaUrl(brand.logo) ? (
                        <Image className='brand-chip-logo' src={resolveLocalMediaUrl(brand.logo)} mode='aspectFit' />
                      ) : (
                        <View className='brand-chip-icon'>
                          <AppIcon name='store' tone='muted' />
                        </View>
                      )}
                      <Text className='brand-chip-name'>{brand.name}</Text>
                    </View>
                  )
                })}
              </View>
        ...
        <View className='store-category-section product-section'>
          <View className='section-header'>
            <View>
              <Text className='section-kicker'>{selectedBrandName ? '品牌商品' : '分类商品'}</Text>
              <Text className='section-title'>{selectedBrandName || routeCategoryName}</Text>
            </View>
            <Text className='section-count'>{productTotal} 件</Text>
          </View>
