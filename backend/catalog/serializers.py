...
from .models import Category, Brand, Product, ProductSKU, MediaImage, SearchLog, HomeBanner, SpecialZone, Case, CaseDetailBlock
...
class SpecialZoneSerializer(serializers.ModelSerializer):
    ...
    def get_image_url(self, obj: SpecialZone):
        request = self.context.get('request')
        url = obj.image.file.url if obj.image and obj.image.file else ''
        if not url:
            return ''
