...
- 商品服务：`frontend/src/services/product.ts:4`
  ...
  - `getProducts` 列表（分页/排序/搜索/专区筛选）`frontend/src/services/product.ts:13`
    - 筛选参数：`show_in_gift_zone`, `show_in_designer_zone`, `show_in_best_seller_zone`, `show_in_promotion_zone`
  - `getProductDetail` 详情 `frontend/src/services/product.ts:26`
...
- 客服支持服务：`frontend/src/services/support.ts:35`
  - `getMessages` 消息列表（支持增量拉取 `after` 参数）`frontend/src/services/support.ts:37`
  - `triggerAutoReply` 触发自动回复（支持 `mark_entered` 标记用户进入）`frontend/src/services/support.ts:40`
  - `sendMessage` 发送消息（支持文本、图片/视频附件、关联订单/商品）`frontend/src/services/support.ts:47`
    - 消息类型：`text`, `card`, `quick_buttons`
    - 附件类型：`image`, `video`
...
- 支付服务：`frontend/src/services/payment.ts:4`
  ...
  - `getPaymentDetail` 详情 `frontend/src/services/payment.ts:28`
  - `startPayment` 开始支付（返回支付参数）`frontend/src/services/payment.ts:33`
  - `syncPayment` 同步支付状态（查单）`frontend/src/services/payment.ts:38`
  - `failPayment`/`cancelPayment`/`expirePayment` 支付失败/取消/过期 `frontend/src/services/payment.ts:43`
...
- 信用账户与账务：`frontend/src/services/credit.ts:66`
  - `getMyAccount` 我的信用账户 `frontend/src/services/credit.ts:68`
  - `getMyStatements`/`getAllMyStatements` 我的对账单列表 `frontend/src/services/credit.ts:73`
  - `getStatementDetail` 对账单详情 `frontend/src/services/credit.ts:82`
  - `getMyTransactions`/`getAllMyTransactions` 我的交易记录 `frontend/src/services/credit.ts:87`
  - `confirmStatement` 确认对账单 `frontend/src/services/credit.ts:96`
  - 数据结构：`CreditAccount`, `AccountStatement`, `AccountTransaction`
...
- 分类页（`/pages/category/index`）：
  - 左侧大类导航 + 右侧子类/品项展示
  - 左侧保留“全部商品”入口；选中后右侧直接展示热门品牌/热门分类，不再额外显示顶部“全部商品”卡片
  - 首页“品类专区”点击“更多”跳转至此页（显示全部品类）
  - 支持从首页点击具体分类图标跳转并自动选中对应大类
- 搜索页（`/pages/search/index`）：关键词检索、热门关键词、搜索建议；首页和分类页跳转时会对 `keyword` 做 URL 编码，搜索页进入后统一解码并去除首尾空格再请求商品列表，避免中文关键词以 `%E6...` 形式展示或查询。
- 商品卡片、商品列表和商品详情会识别 `store_type=partner` 的合作店铺展示商品：保留价格、图片和详情展示，但显示“仅展示”，隐藏或禁用加入购物车/立即购买入口。
- 商品详情页读取 `product_attachments` 展示“资料下载”；仅当附件数组非空时显示，点击 PDF 附件后下载并用小程序文档能力打开，名称优先使用后台配置，缺省时回退原始文件名，再回退 URL 文件名。
- 购物车（`/pages/cart/index`）：按店铺分组展示，支持店铺全选、单品选择、数量调整、移除、清空与结算；结算栏固定并紧贴底部 TabBar 上方，商品滚动内容仅预留必要避让空间，避免遮挡最后一组商品；已下架、规格下架或库存不足商品会显示不可结算原因并排除在结算选择外。
...
- 个人中心（`/pages/profile/index`）：资料展示与编辑、地址管理入口；资料编辑页选择头像图片后先本地预览，保存时通过现有媒体上传接口获取图片 URL，再写入 `/user/profile/` 的 `avatar_url`。
- 菜单调整：客服支持入口位于“信用账户”下方，问题建议入口位于“客服支持”之后（经销商用户显示为：收货地址 → 经销商认证 → 信用账户 → 客服支持 → 问题建议）。
- 客服支持系统：
  - **入口**：个人中心 -> 客服支持（直接进入聊天页面，不再有工单列表）
  - **店铺归属**：个人中心客服默认主店；店铺主页、店铺商品详情进入客服时会携带 `store_id`，订单/商品选择页也按当前 `store_id` 过滤。
  - **展示名称**：用户侧把后台回复统一显示为“商家客服”；自动回复按当前 `store_id` 使用对应店铺模板。
  - **功能**：
    - 实时聊天（`/pages/support-chat/index`）：与客服进行实时对话
    - 消息发送：支持文本、图片、视频、关联订单、关联商品
  - **优化特性**：
    - **增量轮询**：使用 `after` 参数仅拉取新消息，返回重复消息时不更新列表或滚动页面，避免聊天页持续刷新。
    ...
    - **本地缓存**：消息按 `store_id` 缓存在本地存储，进入页面秒开
    - **离线支持**：断网时发送消息按 `store_id` 存入离线队列，网络恢复后自动重试
    - **乐观更新**：发送消息立即上屏，提升体验
...
## 店铺首页共用模板
- 首页合作方店铺入口本身不是动态专区；文案通过 `GET /api/stores/public/partner-entry-config/` 读取独立合作方入口配置。在商家后台“平台管理 -> 店铺管理 -> 首页配置”配置；未配置时回退到“战略伙伴/供应链伙伴/供应链战略合作伙伴”等默认文案。
- 店铺详情页 `/pages/store-detail/index?id=<store_id>` 是多店铺共用模板，展示真实经营店铺的公司名称、Logo、封面、图片轮播、产品类别入口、产品大图、价格、库存、动态专区和商品摘要；店铺名称与 Logo 在品牌信息卡内居中对齐，长名称最多展示两行；品牌分类不在首页默认堆叠展示。
- 只有 `is_visible=true`、`show_on_home=true` 且启用中的合作方店铺会出现在公开店铺入口；`is_visible=false` 的合作店铺在小程序公开侧同时隐藏店铺详情、商品、分类、品牌、轮播和专区信息，`show_on_home=false` 仅隐藏首页/列表入口。
...
- 商品卡片和产品大图都使用后端返回的最终价格与库存；价格继续复用现有客户分组逻辑，但用户端不展示客户分组名称或价格身份。
