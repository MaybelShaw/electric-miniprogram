...
- 迁移时报 `relation "catalog_specialzone" already exists` 或 `column "store_id" does not exist`：更新到包含 `catalog.0038` 兼容修复的版本后重新构建并启动 `backend`。该迁移会跳过已存在的动态专区表、字段、索引和约束，并补齐缺失字段（包括旧表缺失的 `store_id`），适用于表已存在但 `django_migrations` 未记录 `catalog.0038` 的生产库。
