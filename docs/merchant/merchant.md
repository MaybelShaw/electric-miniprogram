...
- 商品新增/编辑表单支持上传 PDF 附件（最多 10 个、单个最大 20MB），可设置前端显示名称；删除附件并提交商品后，后端会清理未被其他商品引用的服务器文件。
...
- 能力概览：
  ...
  - 实时聊天：基于轮询机制的实时消息收发（`GET /support/chat/` + 增量拉取 `after` 参数）。
  - 发送消息：支持文本、**图片**与**视频**附件，以及关联**订单**或**商品**（`POST /support/chat/`）。
  - 自动回复：后台聊天页会对当前可访问会话触发自动回复；小程序用户侧进入会话或发送消息时也会触发。
...
- 店铺分账：
  - 店铺支付配置：平台管理员维护合作方 `wechat_mch_id` 等历史兼容信息；当前合作店铺商品仅展示，不能产生新订单或新的合作店铺分账流水。
  - 商户后台店铺分账页：平台管理员仅在当前选择店铺为主店铺时可查看内部流水、更新到期冻结流水或标记人工结算。
  - 当前店铺分账仅作为内部记录和人工结算依据，不调用微信分账接口。

## 权限与认证
- 登录态判断：`merchant/src/App.tsx:16`
- 登录接口：`/admin/login/`（密码登录，返回 JWT）
- 请求头：`Authorization: Bearer <token>`
- 平台管理员可使用顶部店铺选择器切换经营店铺；选择合作店铺时会隐藏订单、发票、销售统计、账务、折扣、库存日志、信用账户和店铺分账等经营菜单。
- 自营店铺管理员显示本店经营菜单；合作方管理员只显示展示配置菜单：商品、SKU、品牌、分类、客户分组、店铺成员、首页轮播、动态专区、媒体库和问题建议。
- 商户管理后台侧边栏中，“客服聊天”“自动回复”“问题建议”统一展示在“客服管理”分组下。
- 合作方管理员直接访问经营类路由时会回到 `/admin/products`。
