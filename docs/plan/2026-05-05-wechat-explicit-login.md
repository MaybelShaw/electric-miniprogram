...
- [ ] 写失败测试：首次微信快捷登录缺少手机号授权时拒绝创建用户。
- [ ] 写失败测试：带手机号授权时创建或复用用户，返回 access/refresh token。
- [ ] 写失败测试：默认昵称生成且不要求用户首次填写头像昵称。
- [ ] 新增后端登录接口，入参包含 `code` 和手机号授权凭证。
- [ ] 后端封装微信 code 换 openid/session_key 的服务，测试中 mock 外部请求。
- [ ] 后端保存手机号和登录时间。
- [ ] 前端新增显式登录服务，不再在普通浏览时隐式创建账号。
- [ ] 前端新增交易动作守卫，未登录时记录原路径和待恢复动作。
- [ ] 商品分享页允许未登录浏览详情。
- [ ] 点击立即购买时触发登录，登录后回到原商品详情并继续购买动作。
